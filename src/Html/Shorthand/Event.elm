...
{-| This used to exist in elm core but no longer does
-}
...
-}
onInput_ : Json.Decoder a -> (a -> msg) -> Attribute msg
onInput_ dec f =
    on "input" (Json.map f dec)
...
--   on "keypress"
--     (messageDecoder (customDecoder charCode <| Result.fromMaybe "empty character code") <| \r ->
--       case r of
--         Err _ -> Nothing
--         Ok c  -> f c
--     )
...
onKeyboardLost dec f =
    on "blur" (Json.map f dec)
